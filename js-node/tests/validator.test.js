...
test('IdRef: valid string id accepted', () => {
  const r = new Registry();
...
test('IdRef: non-string value rejected with TYPE_MISMATCH', () => {
  const r = new Registry();
  const schema = r.loadDict(idRefSchema());
...
test('IdRef: object value rejected with TYPE_MISMATCH', () => {
  const r = new Registry();
  const schema = r.loadDict(idRefSchema());
  const emp = {
...
test('IdRef: minLength constraint enforced', () => {
  const r = new Registry();
  const schema = r.loadDict(idRefSchema());
...
test('IdRef: array of refType ids valid', () => {
  const r = new Registry();
  const schema = r.loadDict(idRefSchema());
  const emp = {
...
test('IdRef: array item that is not a string rejected', () => {
  const r = new Registry();
  const schema = r.loadDict(idRefSchema());
  const emp = {
...
test('IdRef: unknown refType causes SchemaError at load time', () => {
  let threw = false;
  try {
    const r = new Registry();
    r.loadDict({
    ...
    threw = true;
    assert(e instanceof SchemaError);
  }
  assert(threw, 'expected SchemaError for unknown refType');
});

test('IdRef: both type and refType on same property causes SchemaError', () => {
  let threw = false;
  try {
    const r = new Registry();
    r.loadDict({
  ...
  assert(threw, 'expected SchemaError for both type and refType');
});

test('IdRef: pattern constraint enforced on id string', () => {
  const r = new Registry();
  const schema = r.loadDict({
  ...
  assertEmpty(validate(good, schema.types.Source, schema, r));

  const bad = { _type: 'Source', targetRef: 'ITEM42' };
  const errs = validate(bad, schema.types.Source, schema, r);
  assert(hasCode(errs, ErrorCode.PATTERN_MISMATCH));
});
